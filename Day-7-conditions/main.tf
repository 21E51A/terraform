/*  variable "aws_region" {
 ...
 provider "aws" {
  ...
  resource "aws_s3_bucket" "dev" {
     bucket = "statefile-configuress14"
...
/*  variable "create_bucket" {
 ...
 resource "aws_s3_bucket" "example" {
   ...
   bucket = "my-terraform-example-12-11"
 }
 */
## Example-3
variable "environment" {
  ...
  type    = bool
  default = false # false = non-prod, true = prod
}
...
resource "aws_instance" "example" {
  ...
  tags = {
    Name = var.environment ? "example-prod-${count.index}" : "example-dev-${count.index}"
  }
} */
